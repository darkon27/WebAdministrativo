DELETE FROM SCI_PERSONA	WHERE IDPERSONA=0
...
--select * from SCI_TICKET
--select * from SCI_MAESTRODETALLE
SELECT  * FROM SCI_USUARIO WHERE TRIM(USUARIO)='20153408191'
SELECT  * FROM VIEW_Accesos WHERE USUARIO='20153408191' AND PASSWORD='20153408191  '
SELECT  * FROM VIEW_Accesos WHERE TRIM(USUARIO)='20556939781'


UPDATE SCI_PERSONA SET APEPATERNO='CIA INDUSTRIAL',APEMATERNO='CILSA INDUSTRIAL', NOMBRE='INDUSTRIAL CILSA',NOMBRECOMPLETO='CIA INDUSTRIAL LIMA S A CILSA INDUSTRIAL CILSA'
WHERE DOCUMENTO='20100814324'
...
SELECT  * FROM SCI_MENU
SELECT  * FROM SCI_MENUGRUPO
SELECT  * FROM VIEW_Ticket


SELECT  * FROM VIEW_Garantia
SELECT  * FROM VIEW_GarantiaDetalle
delete FROM SCI_GARANTIADETALLE where IDGARANTIA=4
delete FROM SCI_GARANTIA where IDGARANTIA=4

delete FROM SCI_TICKET where IDTICKET=9
...
ALTER VIEW [dbo].[VIEW_Accesos]
AS
SELECT P.IDPERSONA, D.NOMBRE DESTIPOUSUARIO, p.NOMBRECOMPLETO , ISNULL( p.APEPATERNO,'') APEPATERNO,  ISNULL( p.APEMATERNO,'')APEMATERNO, ISNULL( p.NOMBRE,'')NOMBRE,
p.DOCUMENTO ,p.TIPODOCUMENTO,p.EMAIL ,  U.* , case U.estado when 1 then 'Activo' else 'Inactivo' end DesEstado
FROM SCI_USUARIO U WITH(NOLOCK)
LEFT join  SCI_MAESTRODETALLE D  WITH(NOLOCK) ON D.IDMAESTRO=5 AND   U.TIPOUSUARIO=D.CODIGO 
INNER join SCI_PERSONA p WITH(NOLOCK) on p.IDPERSONA = U.IDUSUARIO
